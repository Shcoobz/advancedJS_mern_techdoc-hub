...
function EditNoteForm({ note, users }) {
  ...
  function onUserIdChanged(e) {
    return setUserId(e.target.value);
  }

  const canSave = [title, text, userId].every(Boolean) && !isLoading;

  async function onSaveNoteClicked(e) {
    if (canSave) {
  ...
  const created = new Date(note.createdAt).toLocaleString('de-AT', {
    day: 'numeric',
    month: 'long',
    year: 'numeric',
    hour: 'numeric',
  ...
  const updated = new Date(note.updatedAt).toLocaleString('de-AT', {
    day: 'numeric',
    month: 'long',
    year: 'numeric',
    hour: 'numeric',
